...
alphas = np.linspace(2000000000,1,20)
folder = "Expression/Test/"


def nonrigidIcpWithReference(sourcemesh,targetmesh,indices):
    ...
    for i, t in enumerate(edges):
        M[i, t[0]] = -1
        M[i, t[1]] = 1
    # Maybe I can change the weight here To modify stiffness of all vertices?
    ...
    o3d.io.write_triangle_mesh(folder + "deformed_mesh"+ "ICP" + ".obj",refined_sourcemesh)
    
    for num_,alpha_stiffness in enumerate(alphas):
        
        print("step- {}/20".format(num_))
        print("Stiffness is {}".format(alpha_stiffness))
        
        # For every stiffness, iterate 3 times
        for i in range(3):
            ...
            distances, indices_discard = knnsearch.kneighbors(vertsTransformed)
            ...
            # kron_M_G = kron_M_G1  -  ((num_+1)/20) * kron_M_G2
            # print(kron_M_G)
            ...
            print(n_source_edges)
            print(n_source_verts)
            print(kron_M_G.shape)
